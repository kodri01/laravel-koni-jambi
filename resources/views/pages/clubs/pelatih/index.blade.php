...
@section('content')
    <div class="container-fluid">
        <div class="bg-white rounded p-3 mb-2">
            <h2 class="color-title mt-1 mb-1">Pelatih Club</h2>
        </div>
        <div class="wrapper-table p-3 bg-white rounded">
            <div class="w-100 mt-3 mb-3">
                <div class="row">
                    <div class="col-sm-3">
                        <a class="btn btn-danger" href="{{ route('clubs.index') }}">Back</a>
                    </div>
                    <div class="col-sm-9 text-right">
                        @can('pelatih-create')
                            <a class="btn btn-primary" href="{{ url('clubs/' . $club_id . '/pelatih/create') }}">Tambah
                                Pelatih</a>
                        @endcan
                    </div>
                </div>
                        ...
                        @foreach ($lists as $list)
                            <tr>
                                ...
                                <td>
                                    <div class="d-flex">
                                        @can('pelatih-edit')
                                            <a href="{{ url('clubs/' . $club_id . '/pelatih/edit/' . $list->id) }}"
                                        ...
                                        @can('pelatih-delete')
                                            <form method="POST"
                                                action="{{ url('clubs/' . $club_id . '/pelatih/delete/' . $list->id) }}"
                                                class="m-1">
                                                {{ csrf_field() }}
                                                {{ method_field('delete') }}
